...
import {
  ArrowLeft, CheckCircle2, XCircle, Filter, BookMarked, RotateCcw,
...
export default function PracticeReview() {
  ...
  // Lưu câu sai vào sổ tay khi có subjectId
  useEffect(() => {
    if (!reviewData || savedMistakes) return;
    const { subjectId, examId, questions } = reviewData;
    if (!subjectId || !currentUser?.id) return;

    const wrongOnes = (questions || []).filter(q => !q.isCorrect);
    if (wrongOnes.length > 0) {
      saveMistakes(currentUser.id, subjectId, wrongOnes, examId || null);
    }
    setSavedMistakes(true);
  }, [reviewData?.subjectId, reviewData?.examId, currentUser?.id]);
  ...
  const { title, score, correctCount, totalCount, questions, subjectId } = reviewData;
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 transition-colors duration-200">
      <header className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border-b border-slate-200/80 dark:border-slate-800 sticky top-0 z-10 shadow-sm">
        <div className="max-w-4xl mx-auto p-4 flex items-center justify-between gap-3 flex-wrap">
          <div className="flex items-center gap-4">
            <button
              onClick={() => navigate('/client/dashboard')}
              className="rounded-xl h-10 w-10 border border-slate-200 dark:border-slate-700 hover:bg-slate-100 dark:hover:bg-slate-800 dark:text-slate-300 bg-white dark:bg-slate-800/50 flex items-center justify-center transition-colors"
            >
          ...
          <div className="flex items-center gap-3 flex-wrap">
            <div className="flex items-center gap-4 bg-slate-50 dark:bg-slate-800/80 px-5 py-2.5 rounded-2xl border border-slate-100 dark:border-slate-700">
              <div className="text-center">
                <div className={`text-2xl font-black ${score >= 5 ? 'text-emerald-500' : 'text-red-500'}`}>{score}/10</div>
                <div className="text-[10px] text-slate-400 dark:text-slate-500 font-bold uppercase">Điểm số</div>
              </div>
              <div className="w-px h-8 bg-slate-200 dark:bg-slate-700" />
              <div className="text-center">
                <div className="text-2xl font-black text-emerald-600 dark:text-emerald-400">{correctCount}/{totalCount}</div>
      ...
      <main className="max-w-4xl mx-auto p-6 space-y-6">
        ...
        <div className="flex items-center gap-3 flex-wrap">
          {/* Filter buttons */}
          <div className="flex items-center gap-1.5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-1 shadow-sm">
            <Filter className="h-3.5 w-3.5 text-slate-400 ml-2" />
            {[
              { key: 'all', label: `Tất cả (${questions?.length || 0})` },
              { key: 'wrong', label: `Câu sai (${wrongCount})` },
              { key: 'correct', label: `Câu đúng (${correctCount})` },
            ].map(({ key, label }) => (
              <button
                key={key}
          ...
          {/* Nút luyện lại câu sai — chỉ hiện khi có subjectId và có câu sai */}
          {subjectId && wrongCount > 0 && (
            <Button
              onClick={() => navigate(`/client/practice/mistakes/${subjectId}`, {
                state: {
                  examId: reviewData.examId || null,
                  examTitle: reviewData.title || '',
                }
              })}
              className="font-bold h-10 px-4 rounded-2xl bg-violet-600 hover:bg-violet-700 text-white border-transparent gap-2 shadow-sm text-sm"
            >
          ...
          <Button
            variant="outline"
            onClick={() => navigate('/client/dashboard')}
            className="font-bold h-10 px-4 rounded-2xl border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 bg-transparent gap-2 text-sm ml-auto"
          >
        ...
        {filteredQuestions.length === 0 ? (
          <div className="text-center py-16 text-slate-400 dark:text-slate-500 font-semibold">
            {filter === 'wrong' ? '🎉 Không có câu nào làm sai!' : 'Không có câu nào phù hợp.'}
          </div>
        ) : (
          filteredQuestions.map((q, idx) => {
            const isCorrect = q.isCorrect !== undefined ? q.isCorrect : (q.userAnswer === q.correctAnswer);
            const qType = q.type || 'single';
            // Lấy index gốc trong questions array để hiện đúng số thứ tự
            const originalIdx = (questions || []).findIndex(orig => orig === q || orig.id === q.id);
                  ...
                  <div className="flex gap-4">
                    ...
                    <div className="flex-1">
                      <h3 className="text-base font-bold text-slate-800 dark:text-slate-200 leading-relaxed flex items-start gap-1">
                        <span>Câu {(originalIdx >= 0 ? originalIdx : idx) + 1}:</span>
                        <span dangerouslySetInnerHTML={{ __html: formatQuestionText(q.text || '') }} />
                      </h3>
                        ...
                        {(() => {
                            ...
                            return optsArray.map((opt, oIdx) => {
                              ...
                              return (
                                <div key={oIdx} className={`p-4 rounded-2xl border-2 ${bgColor} ${borderColor} flex items-center gap-3 ${textClass}`}>
                                  ...
                                  <span className="text-sm" dangerouslySetInnerHTML={{ __html: optText || '' }} />
                                  {isCorrectAns && (
                                ...
                                {validPairs.map((p, pIdx) => {
                                  ...
                                  return (
                                    <div key={pIdx} className="flex flex-col sm:flex-row gap-4 items-center bg-white dark:bg-slate-900 p-3 border border-slate-200 dark:border-slate-800 rounded-xl">
                                      <div className="flex-1 text-slate-700 dark:text-slate-300 font-medium">{p.left}</div>
                                  ...
                                  <div className="flex flex-wrap gap-2">
                                    {userWords.length > 0 ? (
                                      userWords.map((word, wIdx) => (
                                        <span key={wIdx} className={`px-4 py-2 border-2 rounded-2xl text-sm font-bold shadow-sm ${isCorrect ? 'border-emerald-200 dark:border-emerald-900 bg-emerald-50 dark:bg-emerald-950/20 text-emerald-800 dark:text-emerald-300' : 'border-red-200 dark:border-red-900 bg-red-50 dark:bg-red-950/20 text-red-800 dark:text-red-300'}`}>
                                          {word}
                                        </span>
                                      ))
                                    ) : (
                                  ...
                                  <div className="pt-3 border-t border-slate-200 dark:border-slate-800">
                                    ...
                                    <div className="flex flex-wrap gap-2">
                                      {correctWords.map((word, wIdx) => (
                                        <span key={wIdx} className="px-4 py-2 border-2 border-emerald-200 dark:border-emerald-900 bg-emerald-50 dark:bg-emerald-950/20 text-emerald-800 dark:text-emerald-300 text-sm font-bold rounded-2xl shadow-sm">
                                          {word}
                                        </span>
                                      ))}
                                    </div>
                          ...
                          if (qType === 'multitruefalse') {
                            ...
                            return (
                              <div className="bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800 p-4 rounded-2xl space-y-3">
                                {stmts.map((stmt, sIdx) => {
                                  const userAns = userMap[sIdx];
                                  const isStmtCorrect = userAns === stmt.correct;
                                  return (
                                    <div key={sIdx} className={`flex flex-col sm:flex-row gap-3 items-start sm:items-center bg-white dark:bg-slate-900 p-3 border-2 rounded-xl ${isStmtCorrect ? 'border-emerald-200 dark:border-emerald-900' : 'border-red-200 dark:border-red-900'}`}>
        ...
        {subjectId && wrongCount > 0 && (
          <div className="sticky bottom-6 flex justify-center">
            <Button
              ...
              className="font-black h-12 px-8 rounded-2xl bg-violet-600 hover:bg-violet-700 text-white border-transparent gap-2 shadow-xl shadow-violet-500/30 text-base"
            >
              <BookMarked className="h-5 w-5" />
              Luyện lại {wrongCount} câu sai ngay →
            </Button>
          </div>
        )}
      </main>
